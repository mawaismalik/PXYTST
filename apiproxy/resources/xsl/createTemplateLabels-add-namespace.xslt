...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/createTemplateLabels))">
						<tns:createTemplateLabels>
							<xsl:apply-templates select="node()|@*"/>
						</tns:createTemplateLabels>
					</xsl:when>
	...
	<xsl:template match="/createTemplateLabels" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='createTemplateLabels') and not(local-name()='Array')]" name="copy-all">
