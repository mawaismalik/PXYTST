...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:getSIMsSMSCurrentCycleUsageDetails>
					...
					<xsl:when test="not(normalize-space(/getSIMsSMSCurrentCycleUsageDetails))">
						<tns:getSIMsSMSCurrentCycleUsageDetails>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsSMSCurrentCycleUsageDetails>
					</xsl:when>
	...
	<xsl:template match="/getSIMsSMSCurrentCycleUsageDetails" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsSMSCurrentCycleUsageDetails') and not(local-name()='Array')]" name="copy-all">
