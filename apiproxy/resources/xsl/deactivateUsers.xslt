...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/deactivateUsers))">
						<tns:deactivateUsers>
							<xsl:apply-templates select="node()|@*"/>
						</tns:deactivateUsers>
					</xsl:when>
	...
	<xsl:template match="/deactivateUsers" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='deactivateUsers') and not(local-name()='Array')]" name="copy-all">
