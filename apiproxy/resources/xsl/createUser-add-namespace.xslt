...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/createUser))">
						<tns:createUser>
							<xsl:apply-templates select="node()|@*"/>
						</tns:createUser>
					</xsl:when>
	...
	<xsl:template match="/createUser" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='createUser') and not(local-name()='Array')]" name="copy-all">
