...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/getSIMsOnlineInfo))">
						<tns:getSIMsOnlineInfo>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsOnlineInfo>
					</xsl:when>
	...
	<xsl:template match="/getSIMsOnlineInfo" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsOnlineInfo') and not(local-name()='Array')]" name="copy-all">
