...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/bulkSMSSend))">
						<tns:bulkSMSSend>
							<xsl:apply-templates select="node()|@*"/>
						</tns:bulkSMSSend>
					</xsl:when>
	...
	<xsl:template match="/bulkSMSSend" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='bulkSMSSend') and not(local-name()='Array')]" name="copy-all">
