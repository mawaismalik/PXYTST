...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:getSIMsCurrentCycleDataUsageDetails>
					...
					<xsl:when test="not(normalize-space(/getSIMsCurrentCycleDataUsageDetails))">
						<tns:getSIMsCurrentCycleDataUsageDetails>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsCurrentCycleDataUsageDetails>
					</xsl:when>
	...
	<xsl:template match="/getSIMsCurrentCycleDataUsageDetails" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsCurrentCycleDataUsageDetails') and not(local-name()='Array')]" name="copy-all">
