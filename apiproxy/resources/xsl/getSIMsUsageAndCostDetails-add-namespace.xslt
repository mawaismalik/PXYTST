...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:getSIMsUsageAndCostDetails>
					...
					<xsl:when test="not(normalize-space(/getSIMsUsageAndCostDetails))">
						<tns:getSIMsUsageAndCostDetails>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsUsageAndCostDetails>
					</xsl:when>
	...
	<xsl:template match="/getSIMsUsageAndCostDetails" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsUsageAndCostDetails') and not(local-name()='Array')]" name="copy-all">
