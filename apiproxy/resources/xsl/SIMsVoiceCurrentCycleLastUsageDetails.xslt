...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:getSIMsVoiceCurrentCycleLastUsageDetails>
					...
					<xsl:when test="not(normalize-space(/getSIMsVoiceCurrentCycleLastUsageDetails))">
						<tns:getSIMsVoiceCurrentCycleLastUsageDetails>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsVoiceCurrentCycleLastUsageDetails>
					</xsl:when>
	...
	<xsl:template match="/getSIMsVoiceCurrentCycleLastUsageDetails" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsVoiceCurrentCycleLastUsageDetails') and not(local-name()='Array')]" name="copy-all">
