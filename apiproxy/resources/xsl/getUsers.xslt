...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/getUsers))">
						<tns:getUsers>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getUsers>
					</xsl:when>
	...
	<xsl:template match="/getUsers" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getUsers') and not(local-name()='Array')]" name="copy-all">
