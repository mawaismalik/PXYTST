...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/removeLabelFromSIM))">
						<tns:removeLabelFromSIM>
							<xsl:apply-templates select="node()|@*"/>
						</tns:removeLabelFromSIM>
					</xsl:when>
	...
	<xsl:template match="/removeLabelFromSIM" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='removeLabelFromSIM') and not(local-name()='Array')]" name="copy-all">
