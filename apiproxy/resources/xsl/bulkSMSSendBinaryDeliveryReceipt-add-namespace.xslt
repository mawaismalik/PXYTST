...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:bulkSMSSendBinaryDeliveryReceipt>
					...
					<xsl:when test="not(normalize-space(/bulkSMSSendBinaryDeliveryReceipt))">
						<tns:bulkSMSSendBinaryDeliveryReceipt>
							<xsl:apply-templates select="node()|@*"/>
						</tns:bulkSMSSendBinaryDeliveryReceipt>
					</xsl:when>
	...
	<xsl:template match="/bulkSMSSendBinaryDeliveryReceipt" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='bulkSMSSendBinaryDeliveryReceipt') and not(local-name()='Array')]" name="copy-all">
