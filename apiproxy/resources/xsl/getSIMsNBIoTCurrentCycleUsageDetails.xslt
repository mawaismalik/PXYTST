...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="normalize-space(/Root)">
						<tns:getSIMsNBIoTCurrentCycleUsageDetails>
					...
					<xsl:when test="not(normalize-space(/getSIMsNBIoTCurrentCycleUsageDetails))">
						<tns:getSIMsNBIoTCurrentCycleUsageDetails>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getSIMsNBIoTCurrentCycleUsageDetails>
					</xsl:when>
	...
	<xsl:template match="/getSIMsNBIoTCurrentCycleUsageDetails" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getSIMsNBIoTCurrentCycleUsageDetails') and not(local-name()='Array')]" name="copy-all">
