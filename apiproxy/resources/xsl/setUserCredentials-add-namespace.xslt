...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/setUserCredentials))">
						<tns:setUserCredentials>
							<xsl:apply-templates select="node()|@*"/>
						</tns:setUserCredentials>
					</xsl:when>
	...
	<xsl:template match="/setUserCredentials" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='setUserCredentials') and not(local-name()='Array')]" name="copy-all">
