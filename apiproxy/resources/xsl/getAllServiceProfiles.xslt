...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://webservice.stc.comarch.com/" version="1.0">
		...
		<soapenv:Envelope>
			...
			<soapenv:Body>
				<xsl:choose>
					...
					<xsl:when test="not(normalize-space(/getAllServiceProfiles))">
						<tns:getAllServiceProfiles>
							<xsl:apply-templates select="node()|@*"/>
						</tns:getAllServiceProfiles>
					</xsl:when>
	...
	<xsl:template match="/getAllServiceProfiles" name="copy-root">
	...
	<xsl:template match="*[not(local-name()='getAllServiceProfiles') and not(local-name()='Array')]" name="copy-all">
